...
VulkanRenderPipeline::VulkanRenderPipeline(const RenderPipelineDescriptor &desc, const VkDevice device)
    ...
    , m_device(device)
{
    const VulkanShader *vertex_shader = static_cast<const VulkanShader *>(desc.vertex_shader);
    ...
    const VulkanShader *fragment_shader = static_cast<const VulkanShader *>(desc.fragment_shader);
    ...
    const VkPipelineInputAssemblyStateCreateInfo input_assembly_state_create_info = {
        ...
        .pNext = nullptr,
        .flags = 0,
        .topology = conversion::to_vk_primitive_topology(desc.primitive_state.topology),
    ...
    VkPipelineRasterizationStateCreateInfo rasterization_state_create_info = {
        ...
        .polygonMode = conversion::to_vk_polygon_mode(desc.primitive_state.polygon_mode),
        .cullMode = conversion::to_vk_cull_mode(desc.primitive_state.cull_mode),
        .frontFace = conversion::to_vk_front_face(desc.primitive_state.front_face),
    ...
    const VkPipelineMultisampleStateCreateInfo multisample_state_create_info = {
        ...
        .pNext = nullptr,
        .flags = 0,
        .rasterizationSamples = conversion::to_vk_sample_count(desc.multisample_state.sample_count),
    ...
    if (desc.depth_stencil_state.has_value()) {
        ...
        const VkCompareOp depth_compare_operation
            = conversion::to_vk_compare_operation(depth_stencil_state.depth_compare_operation);
    ...
    for (const ColorAttachmentState &color_attachment_state : desc.color_attachment_states) {
        ...
        const VkPipelineColorBlendAttachmentState color_blend_attachment_state = {
            ...
            .srcColorBlendFactor = conversion::to_vk_blend_factor(blend_state.src_factor),
            .dstColorBlendFactor = conversion::to_vk_blend_factor(blend_state.dst_factor),
            .colorBlendOp = conversion::to_vk_blend_operation(blend_state.operation),
            .srcAlphaBlendFactor = conversion::to_vk_blend_factor(blend_state.alpha_src_factor),
            .dstAlphaBlendFactor = conversion::to_vk_blend_factor(blend_state.alpha_dst_factor),
            .alphaBlendOp = conversion::to_vk_blend_operation(blend_state.alpha_operation),
            .colorWriteMask = conversion::to_vk_color_component(blend_state.color_writes),
        };
    ...
    for (const ColorAttachmentState &color_attachment_state : desc.color_attachment_states) {
        color_attachment_formats.push_back(conversion::to_vk_format(color_attachment_state.format));
    }

    const VkFormat depth_attachment_format = desc.depth_stencil_state.has_value()
        ? conversion::to_vk_format(desc.depth_stencil_state.value().depth_format)
        : VK_FORMAT_UNDEFINED;
    ...
    HE_VK_CHECK(
        vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &graphics_pipeline_create_info, nullptr, &m_raw));
}
