...
VkAttachmentLoadOp map_attachment_load_operation(const LoadOperation load_operation)
{
...
VkAttachmentStoreOp map_attachment_store_operation(const StoreOperation store_operation)
{
...
VkBufferUsageFlags map_buffer_usage_flags(const BitFlags<BufferUsage> buffer_usage_flags)
{
...
VkColorComponentFlags map_color_component_flags(const BitFlags<ColorWrites> color_writes)
{
...
VkCullModeFlags map_cull_mode_flags(const Face face)
{
    switch (face)
    {
    case Face::None:
...
VkImageAspectFlags map_image_aspect_flags(const Format format)
{
    switch (format)
    {
    case Format::R8Unorm:
...
VkImageType map_image_type(const Dimension dimension)
{
    switch (dimension)
    {
...
// FIXME: Add finer texture usage flags so format doesn't need to be passed
VkImageUsageFlags map_image_usage_flags(const BitFlags<TextureUsage> texture_usage_flags, const Format format)
{
...
VkImageViewType map_image_view_type(const Dimension dimension)
{
    switch (dimension)
    {
    case Dimension::Texture1D:
...
VkSamplerAddressMode map_sampler_address_mode(const AddressMode filter)
{
    switch (filter)
    {
...
VkSamplerMipmapMode map_sampler_mipmap_mode(const Filter filter)
{
    switch (filter)
    {
    case Filter::Nearest:
