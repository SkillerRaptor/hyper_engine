...
Camera::Camera(const glm::vec3 position, const f32 yaw, const f32 pitch)
    : m_position { position }
    , m_yaw { yaw }
    , m_pitch { pitch }
{
...
void Camera::process_keyboard(const Movement movement, const f32 delta_time)
{
    const f32 velocity = m_movement_speed * delta_time;
    switch (movement)
    {
...
void Camera::process_mouse_movement(const f32 x_position, const f32 y_position, const bool mouse_button_pressed)
{
    if (m_first_mouse)
    {
        m_last_x = x_position;
    ...
    const f32 x_offset = (x_position - m_last_x) * m_mouse_sensitivity;
    const f32 y_offset = (m_last_y - y_position) * m_mouse_sensitivity;
...
void Camera::process_mouse_scroll(const f32 y_offset)
{
    m_fov -= y_offset;
    if (m_fov < 1.0f)
    {
