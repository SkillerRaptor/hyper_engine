...
impl Renderer {
    ...
    pub fn new(render_context: &RenderContext) -> Result<Self, Error> {
        let (image_available_semaphores, render_finished_semaphores, in_flight_fences) =
            Self::create_sync_objects(&render_context.device.device)?;

        let command_pool = Self::create_command_pool(
            &render_context.instance.instance,
            &render_context.surface.surface_loader,
            render_context.surface.surface,
            render_context.device.physical_device,
            &render_context.device.device,
        )?;

        let command_buffers =
            Self::create_command_buffers(&render_context.device.device, command_pool)?;
        ...
        Ok(Self {
            ...
            swapchain: render_context.swapchain.clone(),
            pipeline: render_context.pipeline.clone(),
        })
    }
