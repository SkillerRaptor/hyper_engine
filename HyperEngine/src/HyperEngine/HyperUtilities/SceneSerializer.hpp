...
namespace HyperUtilities
{
	...
	public:
		SceneSerializer(const HyperCore::Ref<HyperRendering::Scene>& scene);
	...
	private:
		template<typename T>
		void SerializeImplement(HyperECS::Entity entity, nlohmann::ordered_json& json)
		{
		...
		template<typename T>
		void DeserializeImplement(HyperECS::Entity entity, const nlohmann::ordered_json& json)
		{
			HyperECS::Registry& registry = m_Scene->GetRegistry();
